...
export function taskOf(queryId: string): Task {
	...
	throw new Error(`Không nhận ra task từ '${queryId}': tên phải kết thúc bằng -kis, -qa hoặc -trake`);
}

export function validate(q: Query): string[] {
	const errs: string[] = [];
	if (q.answers.length > MAX_ANSWERS) errs.push(`${q.answers.length} đáp án, tối đa ${MAX_ANSWERS}`);
	...
	q.answers.forEach((a, i) => {
		...
		if (q.task === 'trake') {
			if (!q.n_events) errs.push(`${at}: chưa khai báo số events`);
				...
				errs.push(`${at}: ${a.frames.length} frame, cần đúng ${q.n_events}`);
			const ids = a.frames.map((f) => f.frame_id);
			if (ids.some((v, k) => k > 0 && v < ids[k - 1])) errs.push(`${at}: frame chưa theo thứ tự thời gian`);
		...
		if (q.task === 'qa') {
			if (!a.text?.trim()) errs.push(`${at}: thiếu answer`);
				...
				errs.push(`${at}: answer ${a.text.length} ký tự, tối đa ${MAX_ANSWER_CHARS}`);
		}
	});
	return errs;
}
